<template>
  <div class="space-y-6 animate-fade-in">
      <div class="flex justify-between items-center">
          <div>
            <h1 class="text-2xl font-bold tracking-tight text-foreground">{{ t('products.accept_return.title') }}</h1>
            <p class="text-sm text-muted-foreground mt-1">{{ t('products.accept_return.subtitle') }}</p>
          </div>
      </div>
      ...
      <div v-else-if="requests.length === 0" class="bg-card rounded-xl border border-border shadow-sm p-12 text-center">
          ...
          <h3 class="text-lg font-medium text-foreground">{{ t('products.accept_return.no_pending') }}</h3>
          <p class="text-muted-foreground mt-1">{{ t('products.accept_return.all_caught_up') }}</p>
      </div>
      ...
      <div v-else class="space-y-6">
          <div v-for="request in requests" :key="request.id" 
              ...
              <div class="bg-muted/30 px-6 py-4 border-b border-border flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                  <div class="flex items-center gap-4">
                      <div class="bg-card p-2 rounded-lg border border-border shadow-sm">
                          <CornerUpLeft class="w-5 h-5 text-orange-600 dark:text-orange-400" />
                      </div>
                      <div>
                  ...
                  <div class="flex items-center gap-3">
                      <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-500/10 text-orange-800 dark:text-orange-400">
              ...
              <div class="p-6 space-y-6">
                  <!-- Notes -->
                  <div v-if="request.notes" class="bg-orange-500/10 border border-orange-200 dark:border-orange-800 rounded-lg p-4 text-sm text-orange-900 dark:text-orange-400 flex gap-3">
                      <Info class="w-4 h-4 text-orange-600 dark:text-orange-400 shrink-0 mt-0.5" />
                      <div>
